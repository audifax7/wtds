...
import { Cell, District, Province, Sector } from "@prisma/client";
import { addVillage } from "@/actions/add-village";

interface VillagesFormProps {
  districts: District[];
  provinces: Province[];
  sectors: Sector[];
  cells: Cell[];
...
export function VillageForm({
  ...
  const form = useForm<z.infer<typeof AddVillageSchema>>({
  ...
  const onSubmit = (values: z.infer<typeof AddVillageSchema>) => {
    startTransition(() => {
      addVillage(values).then((data) => {
